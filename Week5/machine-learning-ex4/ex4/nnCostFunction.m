...
J = (1 / m) * sum(sum( (-Y .* log(a_super_3)) - ((1-Y) .* log(1 - a_super_3)) ));


% Implementing regularized cost function

regu  = lambda  * (sum(sum( Theta1(:,2:end) .* Theta1(:,2:end) )) + sum(sum( Theta2(:,2:end) .* Theta2(:,2:end))))/(2 * m);
J = J + regu;
